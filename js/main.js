...
$(window).on('load', function(){
    ...
    $('.tabs-tables__link--cpu').click(function () {
        ...
        $('.tabs-tables__tab-content').find("[data-cpu='" + cpu + "']").fadeIn();
    });
    ...
    $('.tabs-tables__link').click(function() {
        ...
        $(this).parent('.nav-item').prev('.nav-item').find('.tabs-tables__link').addClass('prev');
    });
    ...
    if ($('.gauge').length) {
        ...
        $('.btn-start-gauge--js').click(function () {
            ...
            gsap.to('.gauge__progress--path',1,{drawSVG: number + '%',ease: 'none'}, 0);
    ...
    $('#heroFaq .tabs-faq__header-link').on('show.bs.tab', function() {
        $('.tabs-faq__header-link').removeClass('active').attr('aria-selected', 'false');
    })
});
